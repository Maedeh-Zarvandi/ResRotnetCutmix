...
class ResNet(nn.Module):
    ...
    def forward(self, x, out_feat_keys=None):
        ...
        if 'conv1' in out_feat_keys:
            out_feats[out_feat_keys.index('conv1')] = out

        out = self.layer2(out)
        if 'conv2' in out_feat_keys:
            out_feats[out_feat_keys.index('conv2')] = out

        out = self.layer3(out)
        if 'conv3' in out_feat_keys:
            out_feats[out_feat_keys.index('conv3')] = out

        out = self.layer4(out)
        if 'conv4' in out_feat_keys:
            out_feats[out_feat_keys.index('conv4')] = out

        out = F.avg_pool2d(out, 4)
        out = out.view(out.size(0), -1)
        out = self.linear(out)
        if 'classifier' in out_feat_keys:
            out_feats[out_feat_keys.index('classifier')] = out
