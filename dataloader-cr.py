    ...
    def get_iterator(self, epoch=0):
        ...
        if self.unsupervised:
            ...
            def _load_function(idx):
                ...
                rotation_labels = torch.FloatTensor([0])

                img0 = torch.unsqueeze(img0, 0)
                return img0, rotation_labels
            ...
            def _collate_fun(batch):
                ...
                batch[0][:, :, bbx1:bbx2, bby1:bby2] = batch[0][rand_index, :, bbx1:bbx2, bby1:bby2]
                ...
                batch1 = []
                for im in batch[0]:
                    # print("batch[0].shape", batch[0].shape) torch.Size([128, 3, 32, 32])
                    # print("im.shape", im.shape) torch.Size([3, 32, 32])
                    # print("im[0].shape", im[0].shape) torch.Size([32, 32])
                    # print("im[1].shape", im[1].shape) torch.Size([32, 32])
                    # print("im.shape", im.shape) torch.Size([32, 32, 3])
                    # im = im.numpy()
                    rotated_imgs = [
                        torch.Tensor(im),
                        torch.Tensor(rotate_img(im,  90)),
                        torch.Tensor(rotate_img(im, 180)),
                        torch.Tensor(rotate_img(im, 270))
                    ]
                    # print("stack shape", torch.stack(rotated_imgs, dim=0).shape) torch.Size([4, 3, 32, 32])
                    batch0.append(torch.stack(rotated_imgs, dim=0))
                    batch1.append(torch.LongTensor([0, 1, 2, 3]))
                
                batch0_ = torch.stack(batch0, dim=0)
                # print("batch0_.shape", batch0_.shape) torch.Size([128, 4, 3, 32, 32])
                batch0_ = batch0_.view([batch_size*4, channels, height, width])
                # print("batch0_.shape", batch0_.shape) torch.Size([512, 3, 32, 32])

                batch1_ = torch.stack(batch1, dim=0)
                # print("batch1_.shape", batch1_.shape) torch.Size([128, 4])
                batch1_ = batch1_.view([batch_size*4])
                # print("batch1_.shape", batch1_.shape) torch.Size([512])


                target_a = batch1_
                # target_b = batch1_[rand_index]

                return batch0_, target_a, target_a, lam
        else: # supervised mode
            # if in supervised mode define a loader function that given the
            # index of an image it returns the image and its categorical label
            def _load_function(idx):
...
if __name__ == '__main__':
    ...
    for i in range(data.size(0)):
        ...
        #fig = plt.imshow(np.transpose(data[i], (1, 2, 0)))
