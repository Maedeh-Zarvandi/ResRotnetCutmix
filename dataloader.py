    ...
    def get_iterator(self, epoch=0):
        ...
        if self.unsupervised:
            ...
            def _collate_fun(batch):
                ...
                # print("batch[0].shape before", batch[0].shape)
                # print("batch[1].shape before", batch[1].shape)
                batch[0] = batch[0].view([batch_size*rotations, channels, height, width])
                batch[1] = batch[1].view([batch_size*rotations])
                # print("batch[0].shape after", batch[0].shape)
                # print("batch[1].shape after", batch[1].shape)
...
################################## new dataloader for applying cutmix after rotation###################################
    ...
    def get_iterator(self, epoch=0):
        ...
        if self.unsupervised:
            ...
            def _collate_fun(batch):
                ...
                rand_index = torch.randperm(batch_size * 4)#.cuda()
                # print("rand_index.shape", rand_index.shape)

                target_a = batch[1]
                target_b = batch[1][rand_index]
                ...
                return batch[0], target_a, target_b, lam
        else: # supervised mode
            # if in supervised mode define a loader function that given the
            # index of an image it returns the image and its categorical label
            def _load_function(idx):
